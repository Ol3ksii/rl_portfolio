...
class MCTSNode:
    ...
    def backpropagate(self, value: float) -> None:
        ...
        # TODO (1/3): Implement the Backup phase.
        #
        # Steps:
        #   1. Increment self.visit_count by 1.
        #   2. Add ``value`` to self.value_sum  (outcome from self.player's perspective).
        #   3. If this node has a parent, recurse: call self.parent.backpropagate(-value).
        #      The sign flip is essential — the parent is the opponent, so a win
        #      for self is a loss for the parent.
        """
        self.visit_count += 1
...
class MCTSAgent:
    ...
    def _rollout(self, state: TicTacToeState, player: int) -> float:
        ...
        # TODO (2/3): Implement the random rollout (default rollout policy).
        #
        # Steps:
        #   1. Set ``current = player`` to track whose turn it is.
        #   2. While the state is not terminal:
        #        a. Pick a random action from available actions.
        #        b. Apply it.
        #        c. Switch player.
        #   3. After the loop, get the winner.
        #
        # TODO (3/3): Return the outcome from the original ``player``'s perspective.
        #
        #   - Return +1.0 if ``player`` won.
        #   - Return  0.0 if it was a draw .
        #   - Return -1.0 if the opponent won.
        #
        # Note: compare _winner(state) against the original ``player`` argument,
